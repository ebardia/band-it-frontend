      ...
      <header className="bg-white shadow">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-earth-900">Edit Band Profile</h1>
              <p className="text-earth-700 mt-1">{band?.name}</p>
            </div>
            <button
        ...
        <form className="bg-white rounded-xl shadow-sm p-8 space-y-8">
          ...
          <div>
            <label className="block text-sm font-medium text-earth-800 mb-2">
              Tagline
              <span className="text-earth-600 font-normal ml-2">(One compelling sentence)</span>
            </label>
            <input
              type="text"
              value={tagline}
              ...
              maxLength={100}
            />
            <p className="text-xs text-earth-600 mt-1">{tagline.length}/100 characters</p>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-earth-800 mb-2">
              Full Description
              <span className="text-earth-600 font-normal ml-2">(Who you are and what you do)</span>
            </label>
            <textarea
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="block text-sm font-medium text-earth-800">
                Core Values
                <span className="text-earth-600 font-normal ml-2">(4-6 values that guide your work)</span>
              </label>
              <button
                type="button"
                onClick={addValue}
            ...
            <div className="space-y-4">
                  ...
                  <div className="flex items-start gap-4">
                    <div className="flex-1 space-y-3">
                      <input
                        ...
                        className="w-full px-3 py-2 border border-earth-300 rounded-lg focus:ring-2 focus:ring-rust outline-none"
                      ...
                      <textarea
                        ...
                        rows={2}
                        className="w-full px-3 py-2 border border-earth-300 rounded-lg focus:ring-2 focus:ring-rust outline-none resize-none"
              ...
              {coreValues.length === 0 && (
                <p className="text-earth-600 text-sm italic">No values added yet. Click "+ Add Value" to start.</p>
              )}
            </div>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-earth-800 mb-2">
              Decision Guidelines
              <span className="text-earth-600 font-normal ml-2">(How your band makes decisions)</span>
            </label>
            <textarea
          ...
          <div>
            <label className="block text-sm font-medium text-earth-800 mb-2">
              Inclusion Statement
              <span className="text-earth-600 font-normal ml-2">(Your commitment to diversity and inclusion)</span>
            </label>
            <textarea
          ...
          <div>
            <label className="block text-sm font-medium text-earth-800 mb-2">
              Membership Policy
              <span className="text-earth-600 font-normal ml-2">(How people can join)</span>
            </label>
            <textarea
          ...
          <div>
            <label className="block text-sm font-medium text-earth-800 mb-2">
              Upload Images
              <span className="text-earth-600 font-normal ml-2">(Photos to showcase your band)</span>
            </label>
            <input
              type="file"
              accept="image/*"
              ...
              className="w-full px-4 py-3 border border-earth-300 rounded-lg focus:ring-2 focus:ring-rust focus:border-transparent outline-none"
            />
            {uploadingImage && <p className="text-sm text-earth-600 mt-2">Uploading image...</p>}
            <p className="text-xs text-earth-600 mt-1">Max 5MB. Formats: JPG, PNG, GIF, WebP</p>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-earth-800 mb-2">
              Upload Documents
              <span className="text-earth-600 font-normal ml-2">(PDFs, bylaws, reports, etc.)</span>
            </label>
            <input
              type="file"
              ...
              className="w-full px-4 py-3 border border-earth-300 rounded-lg focus:ring-2 focus:ring-rust focus:border-transparent outline-none"
            />
            {uploadingDocument && <p className="text-sm text-earth-600 mt-2">Uploading document...</p>}
            <p className="text-xs text-earth-600 mt-1">Max 10MB. Formats: PDF, DOC, DOCX, TXT, XLSX, XLS, CSV</p>
          </div>
      ...
      {showAIModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-8 max-w-2xl w-full mx-4">
            <h2 className="text-2xl font-bold text-earth-900 mb-4">✨ Generate Profile with AI</h2>
